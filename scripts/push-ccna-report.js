...
async function pushReport() {
    try {
        ...
        const targetId = '6259456645005312';
        ...
        const imgUrl = qData.image ? `https://storage.googleapis.com/micro-enigma-235001.appspot.com/ccna/images/${qData.image}` : null;

        const embed = new EmbedBuilder()
            .setTitle('📑 BÁO CÁO THẨM ĐỊNH CHI TIẾT - ID: ' + targetId)
            .setColor(0x0099FF)
            .addFields(
                { name: '📊 1. TRÍCH XUẤT NỘI DUNG GỐC', value: `**Câu hỏi:** ${qData.text}\n**Ảnh:** ${imgUrl || 'N/A'}\n**Đáp án:**\n${qData.answers.map(a=>(a.correct?'✅ ':'❌ ')+a.text).join('\n')}\n**Giải thích:** ${qData.explanation}` },
                { name: '🔍 2. PHÂN TÍCH REPORT', value: `**Lý do:** Incorrect Answer, Wrong Explanation\n**Ghi chú:** N/A\n**Nhận định:** Khách hàng nghi ngờ kết quả định tuyến mặc định (Default Route).` },
                { name: '🧠 3. PHÂN TÍCH CHUYÊN MÔN & NGUỒN OFFICIAL', value: `**Phân tích:** Trong Cisco routing, S* 0.0.0.0/0 là Default Route (Gateway of last resort). Nếu đích không khớp route cụ thể, nó sẽ đi qua Serial 0/2/0 như hình. Nội dung hoàn toàn chính xác.\n**Nguồn:** [Cisco Official Support](https://www.cisco.com/c/en/us/support/docs/ip/routing-information-protocol-rip/16448-default.html)` },
