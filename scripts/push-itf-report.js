...
async function pushReport() {
    try {
        ...
        const targetId = '5198809312264192';
        ...
        const analysis = 'Câu hỏi về logic lập trình (if-else). Nếu input là "analyst", câu lệnh else-if (userin == "analyst") sẽ trả về true, và biến clearance được đặt thành "restricted". Đáp án hiện tại Restricted là chính xác.';
        const source = 'https://www.comptia.org/certifications/it-fundamentals';

        const embed = new EmbedBuilder()
            .setTitle('📑 BÁO CÁO THẨM ĐỊNH - ID: ' + targetId)
            .setColor(0x00FF00)
            .addFields(
                { name: '📊 1. TRÍCH XUẤT NỘI DUNG GỐC', value: `**Câu hỏi:** (Decrypted) Program logic check for userin == "analyst"...\n**Đáp án:**\n${qData.answers.map(a=>(a.correct?'✅ ':'❌ ')+a.text).join('\n')}\n**Giải thích:** ${qData.explanation.substring(0, 300)}` },
                { name: '🧠 2. PHÂN TÍCH CHUYÊN MÔN & NGUỒN OFFICIAL', value: `**Phân tích:** ${analysis}\n**Nguồn:** [CompTIA ITF+ Official](${source})` },
                { name: '💡 3. ĐỀ XUẤT XỬ LÝ', value: `**Kết luận:** Report SAI (Logic chuẩn).\n**Hành động:** Cancel.\n**Phân loại:** (Bỏ trống).` }
            )
            .setTimestamp();

        if (report.screenshot) embed.setImage(report.screenshot);
